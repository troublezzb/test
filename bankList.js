...
//根据银行卡号前几位，获取银行信息
//用法：getBankInfo('5324582111575275');
function getBankInfo(card){
	var card_str = card.substr(0, 8);
	if (bankList.get(card_str)) {
		console.log(bankList.get(card_str));
		return;
	}

	var card_str = card.substr(0, 6);
	if (bankList.get(card_str)) {
		console.log(bankList.get(card_str));
		return;
	}

	var card_str = card.substr(0, 5);
	if (bankList.get(card_str)) {
		console.log(bankList.get(card_str));
		return;
	}

	var card_str = card.substr(0, 4);
	if (bankList.get(card_str)) {
		console.log(bankList.get(card_str));
		return;
	}
